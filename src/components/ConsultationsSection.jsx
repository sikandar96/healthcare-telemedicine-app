      ...
      <div className="consultation-layout">
        ...
        <div className="doctor-list">
          {doctors.map((doctor) => {
            ...
            return <button className={selectedDoctor === doctorKey ? "doctor-row selected" : "doctor-row"} key={doctorKey} onClick={() => onSelectDoctor(doctor)}><span className="doctor-avatar" style={{ background: doctor.color || "#d9f3ed" }}>{initials}</span><span className="doctor-info"><strong>{doctor.name}</strong><small>{doctor.specialization || doctor.specialty}</small></span><span className="doctor-meta"><b>{doctor.consultationFee ? `$${doctor.consultationFee}` : doctor.fee || "Available"}</b><small><span className="online-dot" /> {doctor.available === false ? "Offline" : doctor.wait || "Now"}</small></span><ArrowUpRight size={17} className="row-arrow" /></button>;
          })}
